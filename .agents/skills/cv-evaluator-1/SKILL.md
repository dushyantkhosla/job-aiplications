...
3. **Stack credibility** — Python (assumed), plus relevant frameworks (e.g. PyTorch, TensorFlow, scikit-learn, Spark, dbt, Airflow) listed *in context* of projects, not just as a skill dump.
...
2. **The tailored CV** — provided as YAML (RenderCV schema). Read `cv.sections` for structured content.

---

## Scoring Sheet

Fill this in based on the CV you review, using the rubric above. Be specific in your comments, citing exact bullet points or sections as evidence.
...
Your evaluation should be output in the following YAML format:

```yaml
role: # Extracted role title from JD
...
Stritcly adhere to this format. Do not add any text outside the YAML block. Be specific and actionable in your feedback, citing exact bullet points or sections as evidence for your comments.